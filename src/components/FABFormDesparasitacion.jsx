...
function DesparasitacionForm() {
  return (
    <>
    <div style={{ textAlign: 'center', paddingTop: '20px', color: '#181E1C', paddingBottom: '20px' }}>
     
    </div><div style={{ maxWidth: '100%', width: '80%', margin: '0 auto' }}> {/* Contenedor padre con ancho máximo y margen para centrar */}
...
export default function FABDes() {
  const [formCounter, setFormCounter] = useState(0);
  const [formList, setFormList] = useState([]);

  const handleFabClick = () => {
    const newFormList = [...formList];
    newFormList.push(<DesparasitacionForm key={formCounter} />);
